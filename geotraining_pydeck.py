...
def main():
    ...
    if 'mapstyle' not in st.session_state:
        st.session_state.mapstyle = "https://basemaps.cartocdn.com/gl/positron-gl-style/style.json"
    ...
    with col3: region = st.selectbox("Country/Group",
                                     ["All", "EU", "EU (no RU, UA, TR)", "NA", "SA", "ME",
    ...
    deck = pdk.Deck(
                    ...
                    ## still looking for good map style to approximate google maps and have english place names1
    ...
    st.pydeck_chart(deck, use_container_width=True, on_select=handle_selection)

    st.subheader("Previous city: " + st.session_state.prevcity["city"].iloc[0] + ", " +
                 st.session_state.prevcity["country"].iloc[0] + ". Population: " +
                 f'{int(st.session_state.prevcity["population"].iloc[0]):,}')
...
@st.cache_data
def getmapstyles():
    url2name = {"https://basemaps.cartocdn.com/gl/voyager-gl-style/style.json": "voyager",
...
def load_cities(countries="All", min=1000000, max=100000000):
    geoguessrcountries = ["Canada", "United States", "Mexico", "Guatemala", "Panama", "Colombia",
                          ...
                          "Kazakhstan", "Mongolia", "Dominican Republic"]
